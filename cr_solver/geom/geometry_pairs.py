...
from . import utils
from .geometry import BoundingBox, Capsule, HalfSpace, Sphere
...
def _halfspace_sphere_dist(
    ...
    """Helper: distance between a halfspace boundary plane and sphere center,
    minus radius."""
    dist = (
        jnp.einsum(
            "...i,...i->...", sphere_pos - halfspace_point, halfspace_normal
        ) - sphere_radius
    )
    return dist


def halfspace_sphere(halfspace: HalfSpace,
                     sphere: Sphere) -> Float[Array, "*batch"]:
...
def halfspace_capsule(halfspace: HalfSpace,
                      capsule: Capsule) -> Float[Array, "*batch"]:
    ...
    dist1 = _halfspace_sphere_dist(
        halfspace_normal, halfspace_point, cap_center + segment_offset,
        cap_radius
    )
...
def sphere_capsule(sphere: Sphere, capsule: Capsule) -> Float[Array, "*batch"]:
    ...
    dist = _sphere_sphere_dist(
        sphere_pos, sphere.radius, pt_on_axis, capsule.radius
    )
    return dist


def capsule_capsule(capsule1: Capsule,
                    capsule2: Capsule) -> Float[Array, "*batch"]:
    ...
    pt1_on_axis, pt2_on_axis = utils.closest_segment_to_segment_points(
        a1, b1, a2, b2
    )
...
def _point_to_bbox_dist(
    ...
    # If point is inside box in all dimensions, return negative distance to
    # closest face
...
def capsule_bounding_box(capsule: Capsule,
                         bbox: BoundingBox) -> Float[Array, "*batch"]:
    ...
    closest_point_on_axis = utils.closest_segment_point(
        cap_a, cap_b, bbox_center
    )
...
def bounding_box_bounding_box(bbox1: BoundingBox,
                              bbox2: BoundingBox) -> Float[Array, "*batch"]:
...
def sphere_bounding_box(sphere: Sphere,
                        bbox: BoundingBox) -> Float[Array, "*batch"]:
    ...
    point_to_box_dist = _point_to_bbox_dist(
        sphere_center, bbox_center, bbox_extents
    )
...
def bounding_box_halfspace(bbox: BoundingBox,
                           halfspace: HalfSpace) -> Float[Array, "*batch"]:
    ...
    # Find the vertex of the bounding box that is furthest in the direction of
    # the halfspace normal
    ...
    # The closest point on the box to the halfspace is center minus projected
    # extent
