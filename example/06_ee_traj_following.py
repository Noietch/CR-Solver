...
def circle_traj(traj_length: int):
    ...
    for i in range(traj_length):
        position = np.array(
            [
                np.cos(i / traj_length * 2 * np.pi),
                np.sin(i / traj_length * 2 * np.pi),
                3.5,
            ]
        )
    ...
    for i in range(traj_length):
        ...
        if side == 0:  # Bottom side: left to right
            position = center + np.array(
                [-side_length / 2 + t * side_length, -side_length / 2, 0]
            )
        elif side == 1:  # Right side: bottom to top
            position = center + np.array(
                [side_length / 2, -side_length / 2 + t * side_length, 0]
            )
...
def main():
    ...
    visualize_pcc_model_3d(
        fk_result,
        target_position=ee_traj[0],
        save_path="visualization/ee_traj_following.png",
