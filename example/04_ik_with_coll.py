...
def main():
    ...
    ik_target_handle = server.scene.add_transform_controls(
        "/ik_target", scale=0.8, position=(0.0, 0.0, robot.config.length * robot.config.num_sections), wxyz=(0, 0, 1, 0)
    )
    ...
    while True:
        ...
        cfg, _ = solve_ik(robot=robot, coll=robot_coll, world_coll_list=[plane_coll, sphere_coll_world_current], target_position=ik_target_handle.position, target_wxyz=ik_target_handle.wxyz)
