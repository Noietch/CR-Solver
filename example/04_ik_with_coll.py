...
from soul.solver import solve_ik_with_collision
...
def main():
    ...
    ik_target_handle = server.scene.add_transform_controls(
        "/ik_target", scale=0.8, position=(0.0, 0.0, robot.config.length * robot.config.num_sections), wxyz=(1, 0, 0, 0)
    )
    ...
    while True:
        ...
        cfg, _ = solve_ik_with_collision(robot, robot_coll, [plane_coll, sphere_coll_world_current], ik_target_handle.position, ik_target_handle.wxyz)
