...
def viser_main():
    # Setup Environment
    robot = CCRobotExtend.from_config("configs/robots/cc_4_extend.json")
    robot_coll = RobotCollision.from_config("configs/robots/cc_4_extend.json")
    ...
    ik_target_handle = server.scene.add_transform_controls(
        ...
        position=(
            0.0,
            0.0,
            (robot.config.upper_limits_length + robot.config.lower_limits_length) / 2 * robot.config.num_sections,
        ),
        wxyz=(1, 0, 0, 0),
    )
    ...
    ik_solver_with_initial_states = jax.jit(solver.solve_ik_with_initial_states)
