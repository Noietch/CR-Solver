...
def voxel_fit_surface_mesh(
    ...
    _, idx = jax.lax.top_k(dist, k=n_spheres, largest=False)

    n_pts = pts[idx]
    n_radius = jnp.full((len(n_pts),), sphere_radius)
    return n_pts, n_radius


def get_voxelgrid_from_mesh(
    mesh: trimesh.Trimesh, n_spheres: int, voxelize_method: str = "ray"
) -> Tuple[Union[jnp.Array, None], Union[jnp.Array, None]]:
    ...
    try:
        ...
        pts = voxel.points
        rad = jnp.ravel([radius for _ in range(len(pts))])
    except:
...
def voxel_fit_mesh(
    ...
    if len(pts) > n_spheres:
        ...
        if len(inside_pts) < n_spheres:
            new_pts = jnp.zeros((n_spheres, 3))
            new_pts[: len(inside_pts)] = inside_pts
            new_radius = jnp.zeros(n_spheres)
...
def voxel_fit_volume_sample_surface_mesh(
    ...
    pts = jnp.concatenate([pts, surface_sample_pts])
    rad = jnp.concatenate([rad, sample_radius])
    return pts, rad
...
def fit_spheres_to_mesh(
    ...
    if n_pts is None or len(n_pts) < 1:
        print("sphere_fit: falling back to sampling surface")
        n_pts, n_radius = sample_even_fit_mesh(mesh, n_spheres, surface_sphere_radius)

    if n_pts is not None and len(n_pts) > n_spheres:
        dist = jnp.linalg.norm(n_pts - jnp.mean(n_pts, axis=-1, keepdims=True), axis=-1)
        _, knn_i = jax.lax.top_k(dist, k=n_spheres, largest=True)
        n_pts = n_pts[knn_i]
        n_radius = jnp.ravel(n_radius)[knn_i.flatten().tolist()].tolist()
    return n_pts, n_radius