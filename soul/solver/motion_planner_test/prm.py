...
class ParallelPRM:
    ...
    def __init__(
        ...
        self._compute_single_collision = jax.jit(
            lambda robot, robot_coll, state, world_obj: 
            robot_coll.compute_world_collision_distance(robot, state, world_obj, 1)
        )
        
        # JIT compiled functions
        self._batch_distance = jax.jit(self._batch_distance_fn)
        self._batch_interpolate = jax.jit(self._batch_interpolate_fn, static_argnums=(2,))
    ...
    def sample_collision_free_nodes(
        ...
        if len(collision_free) < num_samples:
            print(f"Warning: Only sampled {len(collision_free)}/{num_samples} collision-free nodes")
    ...
    def _batch_distance_fn(
        ...
        states2: ConstantCurvatureState
    ) -> jnp.ndarray:
        ...
        theta_dist = jnp.linalg.norm(
            states2.theta - state1.theta[None, :],
            axis=1
        )
        phi_dist = jnp.linalg.norm(
            states2.phi - state1.phi[None, :],
            axis=1
        )
    ...
    def _connect_nearest_neighbors(
        ...
        for (idx1, idx2, dist, i, j), is_valid in zip(edges_to_check[:len(valid_edges)], valid_edges):
            if is_valid:
    ...
    def find_path(
        ...
        start_coll = self._batch_check_collision(
            start.repeat(1, axis=0), world_coll
        )[0]
        goal_coll = self._batch_check_collision(
            goal.repeat(1, axis=0), world_coll
        )[0]
    ...
    def _find_closest_valid_node(
        ...
        valid_indices = [
            ...
            if i not in self.collision_set
        ]
        ...
        valid_states = jax.tree_util.tree_map(
            lambda *xs: jnp.stack(xs),
            *[self.nodes[i] for i in valid_indices]
        )
    ...
    def _update_edge_attempts(self, node_idx: int):
        ...
        if self.edge_attempt_count[node_idx] >= self.options.max_num_blocked_edges_before_discard:
