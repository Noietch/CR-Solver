...
class RRTOptions:
    ...
    use_simple_distance: bool = True  # Use simple L2 distance instead of collision-based
...
class OptimizedRRT:
    ...
    def add_node(self, state: ConstantCurvatureState, parent_idx: Optional[int] = None) -> int:
    ...
    def _batch_check_collision_fn(
        ...
        nan_mask = jnp.any(jnp.isnan(states.theta), axis=1) | jnp.any(jnp.isnan(states.phi), axis=1)
    ...
    def _batch_distance_fn(
        ...
        base_dist = jnp.sum((states2.base_position - state1.base_position[None, :])**2, axis=1)
    ...
    def _steer_fn(
        ...
        # Interpolate
        new_base = (1 - alpha) * from_state.base_position + alpha * to_state.base_position
    ...
    def _batch_steer_fn(
        ...
        base_dist = jnp.sum((to_states.base_position - from_states.base_position)**2, axis=1)
        total_dist = jnp.sqrt(theta_dist + phi_dist + base_dist)
        ...
        new_base = (1 - alpha) * from_states.base_position + alpha * to_states.base_position
    ...
    def _interpolate_path_fn(
        ...
        # Vectorized interpolation
        base_positions = jnp.outer(1 - alphas, state1.base_position) + jnp.outer(alphas, state2.base_position)
    ...
    def sample_goal_biased(
        ...
        goal_mask = jax.random.uniform(subkey1, (num_samples,)) < self.options.goal_sample_rate
    ...
    def find_nearest_node(
        self, query: ConstantCurvatureState
    ) -> int:
    ...
    def check_edge_collision(
        ...
        num_steps = min(5, self.options.edge_interpolation_steps)  # Reduce to 5 for speed
        
        # Manual interpolation to avoid overhead
        for i in range(1, num_steps + 1):
            alpha = i / (num_steps + 1)
            interp_base = (1 - alpha) * state1.base_position + alpha * state2.base_position
    ...
    def is_goal_reached(
        ...
        base_close = jnp.allclose(
            state.base_position, goal.base_position,
    ...
    def _find_approximate_path(
        ...
        if self.check_edge_collision(nearest, goal, world_coll):
            ...
            full_path = jax.tree_util.tree_map(
                lambda p, g: jnp.concatenate([p, g], axis=0),
                full_path,
                goal_expanded
            )
            
            print(f"Approximate path found (distance to goal: {self._simple_distance(nearest, goal):.4f})")
            return full_path
        else:
